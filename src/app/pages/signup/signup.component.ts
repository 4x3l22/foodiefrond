...
export class SignupComponent implements OnInit {
  ...
  constructor(
    ...
    this.usuarioForm = new FormGroup({
      ...
      contrasena: new FormControl(
        null,
        [Validators.required, Validators.minLength(8), this.passwordStrengthiValidator.bind(this)] // Validador personalizado correctamente agregado
      ),
  ...
  passwordStrengthiValidator(control: any){
    const value = control.value || '';
    let strength = 0;

    //Comprobar diferentes casos
  ...
  onFileChange(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (!validImageTypes.includes(file.type)) {
        // Mostrar alerta o manejar el error como prefieras
        alert('Solo se permiten archivos de imagen (JPEG, PNG, GIF).');
        input.value = ''; // Limpiar el campo de entrada
        return;
      }

      // Si es una imagen válida, convertir a Base64
